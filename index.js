...
// Mandhi add
app.post('/addmandhi', async (req, res) => {
    try {
      const { name, description, imageUrl } = req.body;
      const mandhi = new Mandhi({ name, description, imageUrl });
      await mandhi.save();
      res.status(201).json({ message: 'Mandhi added successfully', mandhi });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  });
  ...
  // Mandhi view
  app.get('/viewmandhi', async (req, res) => {
    try {
      const mandhiData = await Mandhi.find();
      res.json(mandhiData);
    } catch (error) {
...
app.delete('/removem/:id', async (req, res) => {
  try {
      let id = req.params.id;
      await Mandhi.findByIdAndDelete(id);
      res.send("Deleted");
  } catch (error) {
      res.status(500).send(error);
  }
